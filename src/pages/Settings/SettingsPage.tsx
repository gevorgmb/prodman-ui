...
import { CheckCircle, AlertCircle, Mail, Phone, Loader2 } from 'lucide-react';
...
const SettingsPage: React.FC = () => {
  ...
  const handleSendVerification = async (type: 'email' | 'phone') => {
    ...
    setMessage(null);
    try {
      await apiClient.post(`/api/send-${type}-verification`);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      await apiClient.patch('/api/current-user', { name, phone });
  ...
  return (
    <div className="animate-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <header style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--muted-foreground)' }}>Manage your account settings and verification.</p>
      </header>
